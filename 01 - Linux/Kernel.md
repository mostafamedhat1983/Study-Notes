...
2. System call triggers switch from user space → kernel space
...
Kernel tuning adjusts kernel parameters to optimize performance for specific workloads:
...
The `/proc` file system interfaces with kernel data structures, allowing runtime parameter adjustments.
...
- Kernel runs with highest privilege (Ring 0)
- Kernel vulnerabilities can bypass all user-space security
- Kernel-level exploits provide complete system control
- Containers share the host kernel (kernel exploit affects all containers)
