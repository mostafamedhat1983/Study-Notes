...
**uname**  shows basic system information, mainly about the kernel and system architecture
...
- `touch -d "date time" file` → set a specific date and time using a string.
- `touch -t YYYYMMDDhhmm file` → set a specific timestamp.
- `touch -r ref_file file` → copy the timestamp from another file.
...
- `cp -i file1 file2` → ask before overwrite.
...
- `cp -p file1 file2` → preserve permissions, ownership, and timestamps.
- `cp -a src dest` → archive mode; preserve attributes and copy recursively.
...
- `rm -f file` → force removal and never prompt.
...
**file** shows what type of file something is by examining its contents, not just its filename or extension.
...
- `ln -f target link` → remove existing destination file first.
- `ln -i target link` → ask before replacing destination.
- `ln -v target link` → show each link created.
- `ln -r -s target link` → create a relative symbolic link.
- `ln -t dir target` → create link inside a target directory.
- `ln -T target link` → treat link name as a normal file, not a directory.

---

`netstat` and `ss` are both Linux tools to inspect active sockets, connections, and listening ports, but `ss` is the modern, faster replacement for the now‑deprecated `netstat`.
...
|Output control|More compact, “friendlier” layout|More verbose, richer filtering|
|Feature depth|Limited socket‑level detail|More advanced (timers, filters, per‑socket details)|
...
`df` (disk free) shows disk space usage on mounted file systems: total, used, and available space plus the percentage used for each mount point.
...
`lsof` stands for **“List Open Files”**: it shows **all files currently opened by processes** on a Linux system, plus **network connections, sockets, pipes, devices, and more**.

## What `lsof` is for

- See which process is using a file, directory, or port.
...
**less** and **more** are Linux pager commands used to read text one screen at a time. `more` is basic paging, while `less` is a more powerful pager with better navigation and search.

**less** is an improved pager that includes the main features of **more** plus extra navigation and search capabilities. It is generally preferred because it supports moving through content more flexibly and is better suited for larger files.

## Common shortcuts

For both commands, `q` quits the pager. In `less`, searching is a major feature, commonly used with `/pattern` to find text inside the file.

---

**curl** is a command‑line tool for transferring data from or to a server using URLs.

## What `curl` is used for

- **Fetch web pages or API responses**: `curl https://httpbin.org/get`
...
|`-I`|Fetch only headers.|`curl -I https://example.com`|
|`-X METHOD`|Use a specific HTTP method.|`curl -X POST ...`|
|`-H "Header: value"`|Add custom HTTP headers.|`curl -H "Authorization: Bearer token" ...`|
...
`du` = **disk usage**; shows how much space files and directories use. It works recursively by default for directories.
...
- `du -h <path>` → show sizes in human-readable units like KB, MB, GB.
- `du -ah <path>` → show sizes for all files and directories.
- `du -d 1 <path>` or `du --max-depth=1 <path>` → show size one level deep only.
- `du -c <paths>` → show a grand total at the end.
- `du -x <path>` → stay on the same filesystem; skip other mounted filesystems.
- `du --exclude="pattern" <path>` → exclude matching files or directories.
- `du -b <path>` → show size in bytes.
