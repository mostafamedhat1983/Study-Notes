...
tags:
  ...
  - Bash_Script
---
The `read` builtin command reads one line of data from standard input (or a file descriptor) and assigns it to variables . Basic syntax: ```bash read [options] [variable_names]
...
- If more words than variables exist, last variable gets remaining words
    
- If no variable specified, entire line goes to `$REPLY`
...
|`-p "prompt"`|Display prompt before reading|Interactive input: `read -p "Enter name: " name` |
|`-s`|Silent mode (no echo)|Password input |
|`-t <seconds>`|Timeout after N seconds|Prevent infinite wait |
|`-n <N>`|Read exactly N characters|Single keypress: `read -n 1` |
|`-a <array>`|Read into array (word-by-word)|`read -a myarray` |
|`-d <delim>`|Use custom delimiter instead of newline|`read -d ":"` stops at colon |
|`-u <fd>`|Read from file descriptor|`read -u 9 var` reads from FD 9 |
|`-e`|Use readline (with history/editing)|Interactive shells only |
...
- Code injection via escape sequences
    
- Unintended line continuation with `\` at end of line
...
Controls how `read` splits input into variables :
...
**Important:** When `IFS` is non-whitespace, fields are separated by **exactly one character** :
...
|---|---|
|0|Success|
|1|EOF reached or invalid file descriptor |
|2|Invalid option |
|>128|Timeout occurred |

---

## SECURITY & BEST PRACTICES

1. **Always use `-r`**: Prevents escape sequence interpretation
...
5. **Use `-s` for sensitive data**: Passwords should never echo to terminal
